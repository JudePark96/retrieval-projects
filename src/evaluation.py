...
def zero_shot_eval_main(config: Union[BaseConfig, BaselineConfig]) -> None:
  ...
  index_dataset = ImageIndexingDataset('val', preprocess)
  ...
  eval_dataset = CIRRDataset('val', preprocess)
...
def evaluate(dataloader, model, index_features, index_names, device):
  ...
  # [data_size x dim]
  all_predicted_features = torch.cat(all_predicted_features, dim=0)
  all_predicted_features, index_features = all_predicted_features.to(device, non_blocking=True), index_features.to(
    device, non_blocking=True)

  # *** Evaluation phrase. ***
  index_features = F.normalize(index_features, dim=-1)  # .float()

  distances = 1 - (all_predicted_features @ index_features.T)
  sorted_indices = torch.argsort(distances, dim=-1, descending=False).cpu()
  sorted_index_names = np.array(index_names)[sorted_indices]
  ...
  return {
    ...
    'group_recall@1': group_recall_at1,
    'group_recall@2': group_recall_at2,
    'group_recall@3': group_recall_at3,
  }
